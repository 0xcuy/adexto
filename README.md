...
| AdextoCurveFactory | [`0xaA85bc0cceB35B524b6BB730612540Fb88df0f8e`](https://chainscan.0g.ai/address/0xaA85bc0cceB35B524b6BB730612540Fb88df0f8e) | superseded · `VERSION` `0.10.0` · 20,054 B · block 43173642 · still live · created 6 markets, none of which pay a protocol fee |
...
| `AdextoCurveFactory` `0.10.0` on 4 mainnets | **Live, superseded** | Still deployed and still permissionless. Superseded because `0.11.0` adds the protocol fee leg. It created 6 markets on 0G and none elsewhere, and those markets keep trading on their original three-way split. |
...
| Tokens launched | **6 on 0G, none on the other three** | `totalProjectsCount()` returns 6 on the 0G `0.10.0` factory and 0 everywhere else. `allProjects` is append-only and there is no delete, so that 6 can never decrease — three of those launches were test tickers. The site's market index is a separate registry and lists only the markets worth listing. |
