...
| The Graph indexing | **Deployed to Studio, not serving this site** | `adexto-base` and `adexto-arbitrum` are live at `v0.10.0`, both verified synced past the factory's start block with `hasIndexingErrors: false` and zero projects. Not published to the decentralized network, and `SUBGRAPH_URL_*` is deliberately unset, so the app still reads chain state directly. See below. |
...
`SUBGRAPH_URL_*` is deliberately left unset. Pointing the app at an endpoint that has never indexed a single launch would replace direct chain reads with an indexer that has nothing to say. It gets wired after the first real launch is indexed. Nothing is published to the decentralized network yet either — `projects: []` is proof the handlers have never run on real data, which is not the state to publish from.
