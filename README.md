...
| Exercised at launch | **on Monad only, so far.** `$PARCEL` and the delisted `$CURB` read back `agentBound true, agentId 10251`. `$ADEXTO`, `$ADT`, `$ZEEBO`, `$BLOOP` and `$WOMBO` read `agentBound false` — launched without an id, which is the default. The path is proven on mainnet, on one chain, and it is deliberately not switched on everywhere |
...
**Both endpoints currently return nothing, and the reason changed underneath the explanation.** For a long time it was simply that Base and Arbitrum had factories and no markets, so an empty result was the right answer. `$BLOOP` and `$WOMBO` ended that, and the endpoints stayed empty — `_meta` reports Base synced to block `51593493` and Arbitrum to `507372505`, both far past the launches, both with `hasIndexingErrors: false`.

The fault is in the published build rather than in the mappings. `subgraph/networks.json` is generated, and the committed copy had `AdextoFactory` — the `0.11.0` generation — recorded as `0x0000000000000000000000000000000000000000` on every network, with the `startBlock` of the *old* factory. So the manifest that was deployed wired up only the `0.10.0` data source, and `0.10.0` created no markets on Base or Arbitrum. A data source pointed at the zero address indexes nothing and reports itself healthy while doing it, which is exactly the failure mode that makes an indexer worth checking against chain state instead of against its own status field.

Running `npm run networks` regenerates the file with the real `0.11.0` addresses and start blocks. Republishing to Studio is a separate step and has not happened, so **the two endpoints above are stale until it does.** Nothing on the site reads them for the chains that matter: `SUBGRAPH_URL_0G` is empty and 0G trades come straight from RPC logs, and Monad is served by [Envio](envio/README.md) with full history. The Graph cannot serve either of those chains anyway — 0G is absent from its networks registry, and `monad` is listed there without Subgraphs support.
