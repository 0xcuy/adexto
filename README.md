...
| Governance | **Does not work, and cannot be wired up** | Not "nobody has voted yet" — nobody *can*. `castVote` weighs a ballot with `governanceToken.balanceOf(msg.sender)`, and that address is the zero address on Base and Monad and the superseded v1 hook on 0G and Arbitrum. Every vote reverts; `proposalCount` is 0 on all four. **Pointing it at `$ADEXTO` would not help.** `execute` calls `targetContract` with calldata, so a Governor can only do what its own address may already do — and the launch-path contracts contain zero references to a governor and zero setters. Every fee rate is `immutable`, there is no owner. Giving it power means adding an admin surface, which is the thing `/security` says does not exist. Leave it inert or remove it; there is no third option. |
