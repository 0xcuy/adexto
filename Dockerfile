...
#
# NEXT_PUBLIC_CURVE_FACTORY_* was missing from this list. contracts.ts reads it,
# but a Docker build never received it, so the client bundle inlined `null` for
# every curve factory and launches stayed dark no matter what was set on the
# host. Both generations are threaded through now.
ARG NEXT_PUBLIC_CURVE_FACTORY_0G
ARG NEXT_PUBLIC_CURVE_FACTORY_ARBITRUM
ARG NEXT_PUBLIC_CURVE_FACTORY_BASE
ARG NEXT_PUBLIC_CURVE_FACTORY_MONAD
# Superseded curve factories. Threaded through for the same reason as the line
# above: they are read by contracts.ts and inlined at build time, so omitting them
# here makes the verification tables silently drop the contract that six live
# markets were created by, while the host env looks correct.
ARG NEXT_PUBLIC_CURVE_FACTORY_PREV_0G
...
ENV NEXT_PUBLIC_CURVE_FACTORY_ARBITRUM=$NEXT_PUBLIC_CURVE_FACTORY_ARBITRUM
ENV NEXT_PUBLIC_CURVE_FACTORY_BASE=$NEXT_PUBLIC_CURVE_FACTORY_BASE
ENV NEXT_PUBLIC_CURVE_FACTORY_MONAD=$NEXT_PUBLIC_CURVE_FACTORY_MONAD
ENV NEXT_PUBLIC_CURVE_FACTORY_PREV_0G=$NEXT_PUBLIC_CURVE_FACTORY_PREV_0G
