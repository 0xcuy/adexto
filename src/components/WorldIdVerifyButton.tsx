  ...
  return (
    <>
      <button
        ...
        className="px-3 py-1.5 rounded-lg bg-cream-3 hover:bg-cream-3 text-ink font-mono text-[11px] font-bold border border-line flex items-center gap-1 shrink-0 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {busy || fetching ? (
          <>
            <RefreshCw className="w-3 h-3 animate-spin text-accent" /> {fetching ? "Preparing…" : "Verifying…"}
          </>
        ) : (
      ...
      {error && <span className="text-[10px] font-mono text-danger">{error}</span>}
